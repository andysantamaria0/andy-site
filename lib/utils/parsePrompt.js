export function buildParsePrompt({ trip, memberContext, text, legs = [] }) {
  const legsContext = legs.length > 1
    ? `\n- Trip legs (ordered destinations):\n${legs.map((l, i) => `  ${i + 1}. ${l.destination} (${l.start_date || '?'} to ${l.end_date || '?'})`).join('\n')}`
    : '';
...
- Trip dates: ${trip.start_date || 'not set'} to ${trip.end_date || 'not set'}${legsContext}
...
3. **Logistics**: flights, trains, ferries, accommodations, car rentals — with dates, confirmation numbers, airlines, etc. When multiple people share the same booking (e.g. shared ferry, group train ticket), list ALL their names in person_names.
...
Respond with ONLY valid JSON in this exact format:
{
  ...
  "logistics": [
    {
      "person_name": "primary person (for backward compat)",
      "person_names": ["all people sharing this booking"],
      "type": "flight|train|bus|car|ferry|accommodation|other",
      "title": "short description",
      "leg_destination": "which trip leg destination this belongs to, or null",
      "details": {
        "airline": "...",
        "flight_number": "...",
        "confirmation_code": "...",
