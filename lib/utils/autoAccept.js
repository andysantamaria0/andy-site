...
export async function tryAutoAccept(supabase, { inboundEmailId, tripId, parsedData, senderMemberId, members }) {
  ...
  // Deduplicate logistics first — same confirmation code or flight number = duplicate
  const seenLogistics = new Set();
  const dedupedLogistics = [];
  for (const entry of (parsedData.logistics || [])) {
    const conf = (entry.details?.confirmation_code || '').toLowerCase();
    const flight = (entry.details?.flight_number || '').toLowerCase();
    const key = conf || flight || `${entry.title}|${entry.start_time || ''}`;
    if (!seenLogistics.has(key)) {
      seenLogistics.add(key);
      dedupedLogistics.push(entry);
    }
  }
  ...
  for (const entry of dedupedLogistics) {
  ...
  // Deduplicate expenses first — same amount + vendor + date = duplicate
  const seenExpenses = new Set();
  const dedupedExpenses = [];
  for (const expense of (parsedData.expenses || [])) {
    const key = `${expense.amount}|${(expense.vendor || '').toLowerCase()}|${expense.expense_date || ''}`;
