 ...
 * For group messages: look up whatsapp_groups table.
 * For DMs: delegate to SMS detection (same conversation memory).
 */
export async function detectTripForWhatsApp(supabase, { senderPhone, messageText, groupId }) {
  // Group message — look up linked group
  if (groupId) {
    ...
    if (group?.trips) {
      const member = await findMemberInTrip(supabase, group.trips.id, { senderPhone });
      return { trip: group.trips, member, ambiguous: false, candidates: [] };
    }
  }

  // DM — fall through to SMS detection (same phone-based logic)
  return detectTripForSms(supabase, { senderPhone, messageText });
}
