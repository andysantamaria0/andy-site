...
export default function AuthButton() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--v-ivory-dim)', fontSize: '0.75rem' }}>
        <div style={{ flex: 1, height: 1, backgroundColor: 'var(--v-ivory-faint)' }} />
        <span>or</span>
        <div style={{ flex: 1, height: 1, backgroundColor: 'var(--v-ivory-faint)' }} />
      </div>
      ...
      <form onSubmit={handleEmailSignIn} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {error && (
          <div style={{ color: 'var(--v-coral)', fontSize: '0.8125rem' }}>{error}</div>
        )}
        <input
